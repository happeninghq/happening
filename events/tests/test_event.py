...
class TestEvent(TestCase):
    ...
    def test_humanize_1(self):
        ...
        if self.today_day == 0:
            self.assertEqual(event.time_to_string, "today (%s) at 7PM" % e)
        elif self.today_day == 6:
            self.assertEqual(event.time_to_string, "tomorrow (%s) at 7PM" % e)
        else:
            self.assertEqual(event.time_to_string,
    ...
    def test_humanize_14(self):
        ...
        event = mommy.prepare("Event", start=datetime(2011, 0o1, 0o1))
        self.assertEqual(event.time_to_string,
                         "Saturday January 1st, 2011 at 12AM")

        event = mommy.prepare("Event", start=datetime(2011, 0o1, 0o1, 7, 30))
        self.assertEqual(event.time_to_string,
    ...
    def test_previous_event(self):
        ...
        self.assertEqual(event.previous_event, None)

        event2 = mommy.make("Event", start=datetime.now(pytz.utc) -
                            timedelta(days=4))

        self.assertEqual(event.previous_event, event2)
